...
TEST_F(TestMasterService, TestOnInit) {
    ...
    auto first_task =
        dynamic_pointer_cast<ScoreMatrixTask>(service->task_queue_.front());
...
TEST_F(TestMasterService, TestOnNewMessage) {
    ...
    auto current_task =
        dynamic_pointer_cast<ScoreMatrixTask>(service->current_tasks["slave1"]);
    ASSERT_NE(current_task, nullptr);
    ...
    auto front_task =
        dynamic_pointer_cast<ScoreMatrixTask>(service->task_queue_.front());
    ASSERT_NE(front_task, nullptr);
    ASSERT_EQ(front_task->x_, 1);
    ASSERT_EQ(front_task->y_, 0);
    delete service;
    delete controller;
}
